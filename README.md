...
**Production:** [rabbit-gtm.pages.dev](https://rabbit-gtm.pages.dev/)

## Run locally

Run `python3 agent_engine.py --serve --port 3000`, then open `http://127.0.0.1:3000`. The Cloudflare production site above provides same-origin live public research. GitHub Pages is a static preview only.
...
- Research failures are visible. GitHub Pages has no research function, so it remains disconnected instead of producing invented results. Cloudflare Pages is the free production host for the same-origin function. Founder facts and corrections still work on a static preview.
